...
def main(input_main_base='input_main_base.i', input_main='input_main.i',
         input_sam_base='input_sam_base.i', input_sam='input_sam.i',
         input_moltres_base='input_moltres_base.i', input_moltres='input_moltres.i'):
    ...
    # Read base input files
    main_root = pyhit.load(input_main_base)
    moltres_root = pyhit.load(input_moltres_base)
    sam_root = pyhit.load(input_sam_base)
    ...
    inlet = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Components/inlet')
    inlet['m_bc'] = DENSITY * TOTAL_VOL_FLOW_RATE  # kg/s
    inlet['T_bc'] = temperature

    # Outlet Component
    outlet = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Components/outlet')
    outlet['p_bc'] = CORE_OUTLET_PRESSURE  # Pa

    # Lower plenum
    lower_plenum = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Components/lower_plenum')
    ...
    upper_plenum = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Components/upper_plenum')
    ...
    components = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Components')
    ...
    for i in range(nx-1):
        for j in range(nx-1):
            ...
            pipe = moosetree.find(
                sam_root, func=lambda n: n.fullpath == '/Components/' + pipe_name)
            ...
            pipe['n_elems'] = 34 * 8
            # SAM does not support scalar_source_var yet
    ...
    lower_branch = moosetree.find(
        sam_root, lambda n: n.fullpath == '/Components/lower_branch')
    ...
    upper_branch = moosetree.find(
        sam_root, lambda n: n.fullpath == '/Components/upper_branch')
    lower_branch['K'] = "'" + "0 " * 101 + "'"
    ...
    # files
    eos = moosetree.find(sam_root, func=lambda n: n.fullpath == '/EOS/eos')
    ...
    rho_func = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Functions/rho_func')
    ...
    mu_func = moosetree.find(
        sam_root, func=lambda n: n.fullpath == '/Functions/mu_func')
    mu_func.setComment(
    ...
    # BCs
    wall = moosetree.find(
        moltres_root, func=lambda n:n.fullpath == '/BCs/wall')
    boundary = [str(int(n)) for i, n in enumerate(np.linspace(200, 299, 100))
    ...
    graphite = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Materials/graphite')
    graphite['prop_names'] = "'k cp rho'"
    graphite['prop_values'] = f"'{graphite_k} {graphite_cp} {graphite_rho}'"

    # UserObjects & Transfers
    uo = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/UserObjects')
    transfers = moosetree.find(
        main_root, func=lambda n: n.fullpath == '/Transfers')
    kernels = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Kernels')
    for i in range(nx-1):
        for j in range(nx-1):
            if i == center_idx[0] and j == center_idx[0]:
                pass
    ...
    nt_action =  moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Nt')
    nt_action['fission_blocks'] = blocks
    nt_action['pre_blocks'] = blocks
    heat_aux = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/AuxVariables/heat')
    heat_aux['block'] = blocks
    neutron_source_aux = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/AuxVariables/neutron_source')
    ...
    salt_mat = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Materials/salt')
    salt_mat['block'] = channel_blocks
    bnorm = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Postprocessors/bnorm')
    bnorm['block'] = blocks
    total_heat = moosetree.find(
        moltres_root, func=lambda n: n.fullpath == '/Postprocessors/total_heat')
    ...
    pyhit.write(input_main, main_root)
    pyhit.write(input_moltres, moltres_root)
    pyhit.write(input_sam, sam_root)
